...
const menuModeOptions = ref([
  { label: t('view.app-configurator.static'), value: 'static' },
  { label: t('view.app-configurator.overlay'), value: 'overlay' }
]);
...
if (layoutConfig.surface) {
  applyTheme(
    'surface',
    surfaces.value.find((s) => s.name === layoutConfig.surface)
  );
}
onPresetChange();
</script>
...
<template>
  ...
  >
    <div class="flex flex-col gap-4">
      <div>
        <span class="text-sm text-muted-color font-semibold">{{ t('view.app-configurator.primary') }}</span>
      ...
      <div>
        <span class="text-sm text-muted-color font-semibold">{{ t('view.app-configurator.surface') }}</span>
        <div class="pt-2 flex gap-2 flex-wrap justify-between">
          <button
      ...
      <div class="flex flex-col gap-2">
        <span class="text-sm text-muted-color font-semibold">{{ t('view.app-configurator.preset') }}</span>
        <SelectButton v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" />
      </div>
      <div class="flex flex-col gap-2">
        <span class="text-sm text-muted-color font-semibold">{{ t('view.app-configurator.menu-mode') }}</span>
