...
<template>
  <form @submit.prevent="save">
    <div class="flex flex-col gap-6">
      <div>
        <label for="name" class="block font-bold mb-3">{{ $t('username') }}</label>
        <InputText id="name" v-model="user.username" fluid required></InputText>
      </div>

      <div>
        <label for="password" class="block font-bold mb-3">{{ $t('password') }}</label>
        <Password id="password" v-model="user.password" toggleMask fluid required></Password>
      </div>
      <div>
        <label for="role" class="block font-bold mb-3">{{ $t('role') }}</label>
